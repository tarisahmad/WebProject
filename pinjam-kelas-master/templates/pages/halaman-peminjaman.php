...
<table>
	...
	<tr>
		...
		<td><?php echo date('d/m/Y', strtotime($baris['tgl'])); ?></td>
		<td align="center">
			
			<!-- Hanya bisa dilihat oleh 2 [Awal] -->
			<?php if($this->session->userdata('level') == "2"): ?>
				...
				<?php endif; ?>
			<?php endif; ?>
			<!-- Hanya bisa dilihat oleh 2 [Akhir] -->

			<!-- Hanya bisa dilihat oleh 1 atau 3 [Awal] -->
			<?php if($this->session->userdata('level') == "1" OR $this->session->userdata('level') == "3"): ?>
				...
				<?php endif; ?>
			<?php endif; ?>
			<!-- Hanya bisa dilihat oleh 1 atau 3 [Akhir] -->

			<!-- Simbol ceklis hanya bisa dilihat oleh 1 atau 3 [Awal] -->
			<?php if($this->session->userdata('level') == "1" OR $this->session->userdata('level') == "3"): ?>
				<?php if($baris['verifikasi_bp'] == 'OK' && $baris['verifikasi_kabag'] == 'OK'): ?>
				<span style="color: #4CAF50;" class="lnr lnr-checkmark-circle"></span>
				<?php endif; ?>
			<?php endif; ?>
			<!-- Simbol ceklis hanya bisa dilihat oleh 1 atau 3 [Akhir] -->
		</td>
		<td align="center">
			<div>
				...
				<?php if($this->session->userdata('level') == "1" OR $this->session->userdata('level') == "3"): ?>
					<?php if($baris['status_pinjam'] == 'Menunggu' && $this->session->userdata('level') == "1" OR $baris['status_pinjam'] == 'Menunggu' && $this->session->userdata('level') == "3" OR $baris['status_pinjam'] == 'Disetujui' && $this->session->userdata('level') == "1"): ?>
			...
			<?php if($baris['status_pinjam'] != 'Selesai' && $baris['status_pinjam'] != 'Dibatalkan'): ?>
				<div <?php if($baris['status_pinjam'] == 'Disetujui' && $this->session->userdata('level') == "3" OR $this->session->userdata('level') == "2" OR $this->session->userdata('level') == "2") echo "class='sembunyikan'"; ?> class="dropdown-menu" aria-labelledby="dropdown-status">

					<!-- Opsi "Setujui" dan "Batalkan" tampil bila status peminjaman menjadi "Menunggu" [Awal] -->
					<?php if($baris['status_pinjam'] == 'Menunggu'): ?>

						<!-- Opsi "Setujui" hanya tampil bila diakses oleh 1 [Awal] -->
						...
						<?php endif; ?>
						<!-- Opsi "Setujui" hanya tampil bila diakses oleh 1 [Akhir] -->
					...
					<!-- Opsi "Selesai" hanya tampil bila status peminjaman menjadi "Disetujui" dan diakses oleh 1 [Awal] -->
					<?php if($baris['status_pinjam'] == 'Disetujui' && $this->session->userdata('level') == "1"): ?>
					<a class="dropdown-item" href="" data-toggle="modal" data-target="#selesaikan<?php echo $baris['id_peminjaman']; ?>" data-backdrop="static">
						Selesai
					</a>
					<?php endif; ?>
					<!-- Opsi "Selesai" hanya tampil bila status peminjaman menjadi "Disetujui" dan diakses oleh 1 [Akhir] -->
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<ul>
					...
					<li>
						<div><i class="lnr lnr-calendar-full"></i></div><input type="date" name="tgl" placeholder="input tanggal" required />
					</li>
					<br /><hr />
					<li>
			...
			<div class="modal-body">
				...
				<ul>
					...
					<li>
						...
						<div><i class="lnr lnr-text-format"></i></div><input type="date" name="tgl" placeholder="input tanggal" value="<?php echo $baris['tgl']; ?>" required />
					</li>
					<br /><hr />
					<li>
